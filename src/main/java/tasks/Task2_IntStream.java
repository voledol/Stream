...
public class Task2_IntStream {
    ...
    public static Integer multiply(List<Integer> linkedListOfNumbers) {
        ...
        Optional<Integer> prom = Optional.of((linkedListOfNumbers.stream().reduce( (num1, num2) -> num1 * num2)).filter(num -> num <= 100).orElse(-1));
        return prom.get();
    }
    ...
    public static double findAverage(int[] numbers) {
        IntStream stram = IntStream.of(numbers);
        Optional<Double> prom = Optional.ofNullable(stram.average().orElseThrow());
        double result = prom.get();
        return result;
